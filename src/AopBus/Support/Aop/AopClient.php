...
class AopClient
{
        ...
        if (is_array($postFields) && 0 < count($postFields)) {
            ...
            foreach ($postFields as $k => $v) {
                if ("@" != substr($v, 0, 1)) //判断是不是文件上传
                {
                    ...
                    $postBodyString .= "$k=" . urlencode($this->characet($v,
                            $this->postCharset)) . "&";
    ...
    public function execute(
        ...
        foreach ($sysParams as $sysParamKey => $sysParamValue) {
            $requestUrl .= "$sysParamKey=" . urlencode($this->characet($sysParamValue,
                    $this->postCharset)) . "&";
        }
    ...
    public function exec($paramsArray)
    {
        ...
        $requestClassName = ucfirst($inflector->camelize(substr($paramsArray["method"],
                7))) . "Request";
    ...
    function parserJSONSignSource($request, $responseContent)
    {
        ...
        if ($rootIndex > 0) {
            ...
            if ($errorIndex > 0) {
                ...
                return $this->parserJSONSource($responseContent, $this->ERROR_RESPONSE,
                    $errorIndex);
            } else {
    ...
    public function checkResponseSign($request, $signData, $resp, $respObject)
    {
        ...
        if (!$this->checkEmpty($this->alipayPublicKey) || !$this->checkEmpty($this->alipayrsaPublicKey)) {
            ...
            // 获取结果sub_code
            $responseSubCode = $this->parserResponseSubCode($request, $resp, $respObject,
                $this->format);


            if (!$this->checkEmpty($responseSubCode) || ($this->checkEmpty($responseSubCode) && !$this->checkEmpty($signData->sign))) {

                $checkResult = $this->verify($signData->signSourceData, $signData->sign,
                ...
                if (!$checkResult) {
                    ...
                    if (strpos($signData->signSourceData, "\\/") > 0) {
                        ...
                        $signData->signSourceData = str_replace("\\/", "/",
    ...
    private function parserEncryptJSONSignSource($request, $responseContent)
    {
        ...
        if ($rootIndex > 0) {
            ...
            if ($errorIndex > 0) {
                ...
                return $this->parserEncryptJSONItem($responseContent, $this->ERROR_RESPONSE,
                    $errorIndex);
            } else {
    ...
    private function parserEncryptXMLSignSource($request, $responseContent)
    {
        ...
        if ($rootIndex > 0) {
            ...
            if ($errorIndex > 0) {
                ...
                return $this->parserEncryptXMLItem($responseContent, $this->ERROR_RESPONSE,
                    $errorIndex);
            } else {
